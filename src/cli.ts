...
async function dispatch(): Promise<void> {
  switch (args.cmd) {
    ...
    case 'agent': {
      ...
      if (!sub) throw new EngineError(`Usage: msw agent <${AGENT_CHOICES}|install|update|uninstall>`);
      engine.setAgent(sub);
...
async function providerCommand(): Promise<void> {
  ...
  if (sub === 'add-model') {
    if (!rest[0] || !rest[1]) throw new EngineError('Usage: msw provider add-model <供应商> <模型>');
    const model = engine.addModel(rest[0], rest.slice(1).join(' '));
    console.log(`已添加模型 ${model.modelId}`);
    return;
  }
  if (sub === 'rm-model') {
    if (!rest[0] || !rest[1]) throw new EngineError('Usage: msw provider rm-model <供应商> <模型>');
    const model = engine.removeModel(rest[0], rest.slice(1).join(' '));
    console.log(`已删除模型 ${model.modelId}`);
    return;
  }
  if (sub === 'select-model') {
    if (!rest[0] || !rest[1]) throw new EngineError('Usage: msw provider select-model <供应商> <模型>');
...
async function agentInstallCommand(action: 'install' | 'update' | 'uninstall', agentId?: string): Promise<void> {
  const id = agentId || engine.status().state.currentAgent;
  if (!id) throw new EngineError(`Usage: msw agent ${action} <${AGENT_CHOICES}>`);
  const info = await engine.agentInstallInfo(id);
  if (action === 'uninstall') {
    if (!info.installed) throw new EngineError(`${info.name} 未安装`);
    ...
    if (result.code !== 0) throw new EngineError(`${info.name} 卸载失败 (exit ${result.code})`);
  ...
  if (version && !isNpmVersion(version)) throw new EngineError(`无效版本 ${version}`);
  if (action === 'install' && info.installed && !info.outdated && !version) {
    console.log(`${info.name} 已安装 ${info.version || info.bin || ''}`.trim());
    if (info.latest) console.log(color.dim(`最新版本 ${info.latest}`));
    return;
  }
  ...
  if (result.code !== 0) throw new EngineError(`${info.name} ${info.installed ? '更新' : '安装'}失败 (exit ${result.code})`);
  const after = await engine.agentInstallInfo(id);
  if (!after.installed) {
...
function doctor(): void {
  ...
  for (const agent of status.agents) {
    ...
    if (!agent.installed) console.log(color.dim(`           msw agent install ${agent.id}`));
  }
