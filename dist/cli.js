...
import { color, printProfiles, printProviders, printStatus, printSwitch } from "./cli/format.js";
...
async function dispatch() {
    switch (args.cmd) {
        ...
        case 'use': {
            if (!args.args[0])
                throw new EngineError('Usage: msw use <profile|provider|model|agent:model>');
        ...
        case 'model': {
            if (!args.args[0])
                throw new EngineError('Usage: msw model <id>');
...
async function providerCommand() {
    ...
    if (sub === 'add') {
        ...
        const provider = engine.addProvider({
            ...
            wireApi: flag(args, 'wire-api') || undefined,
...
function profileCommand() {
    ...
    if (sub === 'add') {
        if (!rest[0])
            throw new EngineError('Usage: msw profile add <name>');
...
function mcpCommand() {
    ...
    if (sub === 'add') {
        ...
        if (!name)
            throw new EngineError('Usage: msw mcp add --name filesystem --command npx --args -y,@pkg');
