...
import { grokHome } from "../core/paths.js";
import { findTableSpan, getTable, removeTable, stringifyTomlValue, upsertTable, } from "../core/toml.js";
import { payloadModelIds } from "../core/types.js";
...
function modelTableHeader(modelId) {
    return `model.${JSON.stringify(modelId)}`;
}
function mcpTableHeader(name) {
    return `mcp_servers.${JSON.stringify(name)}`;
}
function apiBackend(proto) {
    return proto.wireApi === 'chat' ? 'chat_completions' : 'responses';
}
function openaiProto(provider) {
    const proto = provider.protocols.openai;
    if (!proto)
        throw new Error(`Provider ${provider.id} has no OpenAI protocol for Grok Build`);
    return proto;
}
...
function setModelsDefault(text, modelId) {
...
export const grokBuildAdapter = {
    ...
    parseProbe(input) {
        ...
        if (root) {
            ...
            if (error)
                return { error: asString(error.message) || asString(root.error) || 'Grok Build 报错' };
            ...
            const reply = asString(root.result) ||
                asString(root.text) ||
