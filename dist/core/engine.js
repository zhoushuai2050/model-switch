...
function matchLiveProvider(agentId, live) {
    ...
    if (!url)
        return undefined;
    const hits = providers.filter((item) => Object.values(item.protocols).some((cfg) => normalizeMatchUrl(cfg?.baseUrl, agentId) === url));
    if (hits.length === 1)
        return hits[0];
...
function normalizeMatchUrl(url, agentId) {
    if (!url)
        return '';
