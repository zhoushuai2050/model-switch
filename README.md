...
浏览器打开 http://127.0.0.1:8787 。顶部切 Agent 后，只显示这个 Agent 能用的供应商。工具栏只显示 Agent 名和版本：未安装时点安装，已安装时可以卸载或打开版本管理。版本管理会拉取最近 10 个 npm 版本，用来升级或降级。卡片上可以点选模型、启用、测通、编辑；添加模型会弹出窗口。左下角可切换主题和中文 / English。
